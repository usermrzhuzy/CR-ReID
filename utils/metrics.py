...
# def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):  #数据集celeb用的
#     """Evaluation with market1501 metric.
#     Key: for each query identity, its gallery images from the same camera view are discarded.
#     """
#     num_q, num_g = distmat.shape
#     # 类型转换：确保是 Python int 类型（避免 np.int64 带来的问题）
#     # q_pids = np.asarray([int(pid) for pid in q_pids])
#     # g_pids = np.asarray([int(pid) for pid in g_pids])
#
#     if num_g < max_rank:
#         max_rank = num_g
#         print("Note: number of gallery samples is quite small, got {}".format(num_g))
...
#
#     # 使用 np.take 安全地进行索引
#     matches = (np.take(g_pids, indices) == q_pids[:, np.newaxis]).astype(np.int32)
#
#     all_cmc = []
#     all_AP = []
#     num_valid_q = 0.  # number of valid query
...
#
#         order = indices[q_idx]
#         # 不需要根据相机ID进行排除，直接根据身份匹配
#         remove = np.zeros_like(order, dtype=bool)  # 不移除任何图像
#         keep = np.invert(remove)  # 所有的图库图像都保留
#
#         orig_cmc = matches[q_idx][keep]
#         if not np.any(orig_cmc):
#             continue
...
#         all_AP.append(AP)
#
#     assert num_valid_q > 0, f"Error: all query identities do not appear in gallery. Valid queries: {num_valid_q}"
#
#     all_cmc = np.asarray(all_cmc).astype(np.float32)
#     all_cmc = all_cmc.sum(0) / num_valid_q
#     mAP = np.mean(all_AP)
